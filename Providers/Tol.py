...
def getList(page=None, cat=None):
    ...
    result = []
    pageLink = (
        "https://bpjt.pu.go.id/cctv/cctv_inframe/?id_ruas=" + cat + "&status=online"
    )
    responsePage = requests.request(
        "GET", pageLink, headers=headers, data=payload, verify=False
    )
    ...
    for item in data:
        ...
        if video:
            ...
            result.append(
                {
                    "name": item.find("div", {"class", "text-center"})
                    .text.strip()
                    .rstrip(),
                    "stream": streamUrl,
                    "header": {
                        "Connection": "keep-alive",
                        "Origin": "https://bpjt.pu.go.id/cctv/cctv_inframe",
                        "Referer": "https://bpjt.pu.go.id/cctv/cctv_inframe",
                        "Sec-Fetch-Dest": "document",
                        "Sec-Fetch-Mode": "navigate",
                        "Sec-Fetch-Site": "same-origin",
